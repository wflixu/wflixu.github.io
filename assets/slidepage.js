(function(root, factory) {
  ...
  var utils = {
    ...
    isEqualNode: function(el1, el2) {
      ...
      var equalClass =
        el1.className.replace(/ transition/g, "") ===
        el2.className.replace(/ transition/g, "");
      return equalClass && equalNodeName && equalNodeType && equalHTML;
    },
    isDOM: function(obj) {
      if (
        obj instanceof NodeList ||
        (obj instanceof HTMLCollection && obj.length > 0)
      ) {
        var isTrue = 0;
  ...
  var methods = {
    ...
    resetAnimation: function(index) {
      if (this.opt.useAnimation && this.opt.refresh) {
        ...
        lazys.length > 0 &&
          lazys.forEach(function(element) {
            element.style.display = "none";
          });
      }
    },
    ...
    initEvent: function() {
      ...
      if (this.opt.useSwipe) {
        ...
        this.container.addEventListener(
          "touchmove",
          this.eventHandler.touchMove
        );
        this.container.addEventListener(
          "touchend",
          this.eventHandler.touchEnd,
          supportsPassive ? { passive: true } : false
        );
      }
      ...
      this.container.addEventListener(
        "transitionend",
        this.eventHandler.transitionEnd
      );
    }
  };
  ...
  var slidePage = function(opt) {
    ...
    this.container = utils.isDOM(this.opt.slideContainer)
      ? this.opt.slideContainer
      : document.querySelector(this.opt.slideContainer);
    this.items = utils.isDOM(this.opt.slidePages)
      ? this.opt.slidePages
      : document.querySelectorAll(this.opt.slidePages);
  ...
  slidePage.prototype.update = function(pages) {
    ...
    this.canNext = true;
    this.canPrev = true;
    var newItems = utils.isDOM(pages)
      ? pages
      : document.querySelectorAll(this.opt.slidePages);
    for (var i = 0, len = newItems.length; i < len; i++) {
      ...
      if (
        ...
        utils.isEqualNode(this.items[this.page - 1], newItems[i])
      ) {
        this.page = i + 1;
        break;
      }
